...
class _AnimatedToggleSwitchPageState extends State<AnimatedToggleSwitchPage> {
  ...
  bool positive = false;
  bool loading = false;
  static const green = Color(0xFF45CC0D);
  @override
    ...
    return BasePage(
      ...
      widgets: [
          ...
          child: IconTheme.merge(
            ...
            child: AnimatedToggleSwitch<bool>.dual(
              ...
              customStyleBuilder: (context, local, global) {
                if (global.position <= 0.0) return ToggleStyle(backgroundColor: Colors.red[800]);
                return ToggleStyle(
              ...
              onChanged: (b) => setState(() => positive = b),
              iconBuilder: (value) =>
                  value ? Icon(Icons.power_outlined, color: green, size: 32.0) : Icon(Icons.power_settings_new_rounded, color: Colors.red[800], size: 32.0),
        ...
        DefaultTextStyle.merge(
          ...
          child: IconTheme.merge(
            ...
            child: AnimatedToggleSwitch<bool>.dual(
              ...
              iconBuilder: (value) => value ? Icon(Icons.coronavirus_rounded) : Icon(Icons.tag_faces_rounded),
              textBuilder: (value) => value ? Center(child: Text('Oh no...', style: const TextStyle(color: Colors.white))) : Center(child: Text('Nice :)')),
            ),
          ),
        ),
        SizedBox(height: 16.0),
        AnimatedToggleSwitch<bool>.dual(
          ...
          borderWidth: 6.0,
          height: 60.0,
          loadingIconBuilder: (context, global) => CupertinoActivityIndicator(color: Color.lerp(Colors.red, Colors.green, global.position)),
          onChanged: (b) {
            setState(() => positive = b);
            return Future<dynamic>.delayed(Duration(seconds: 2));
          },
          iconBuilder: (value) =>
              value ? Icon(Icons.power_outlined, color: Colors.green, size: 32.0) : Icon(Icons.power_settings_new_rounded, color: Colors.red, size: 32.0),
        ...
        Padding(
          ...
          child: Text.rich(
            TextSpan(children: [
              ...
              TextSpan(text: 'lite_rolling_switch', style: tappableTextStyle, recognizer: TapGestureRecognizer()..onTap = () => launchUrl(liteRollingSwitchUrl))
            ]),
          ),
        ),
        DefaultTextStyle.merge(
          ...
          child: IconTheme.merge(
            ...
            child: AnimatedToggleSwitch<bool>.dual(
              ...
              iconBuilder: (value) => value
                  ? Icon(Icons.lightbulb_outline_rounded, color: green, size: 28.0)
                  : Icon(Icons.power_settings_new_rounded, color: Colors.red[800], size: 30.0),
              textBuilder: (value) => value
                  ? Align(alignment: AlignmentDirectional.centerStart, child: Text('Active'))
                  : Align(alignment: AlignmentDirectional.centerEnd, child: Text('Inactive')),
            ),
          ),
        ),
        Padding(
          ...
          child: Text.rich(
            TextSpan(children: [
              TextSpan(text: 'Switch inspired by package '),
              TextSpan(text: 'toggle_switch', style: tappableTextStyle, recognizer: TapGestureRecognizer()..onTap = () => launchUrl(toggleSwitchUrl))
            ]),
          ),
        ),
        ...
        Padding(
          ...
          child: Text.rich(
            TextSpan(children: [
              ...
              TextSpan(text: 'Crazy Switch', style: tappableTextStyle, recognizer: TapGestureRecognizer()..onTap = () => launchUrl(crazySwitchUrl))
            ]),
          ),
        ),
        Padding(
        ...
        Padding(
          ...
          child: Text.rich(
            TextSpan(children: [
              TextSpan(text: 'Switch inspired by '),
              TextSpan(text: 'load_switch', style: tappableTextStyle, recognizer: TapGestureRecognizer()..onTap = () => launchUrl(loadSwitchUrl))
            ]),
          ),
        ),
        Padding(
        ...
        AnimatedToggleSwitch<int>.rolling(
          ...
          onChanged: (i) {
            ...
            return Future<Object?>.delayed(Duration(seconds: 3)).then((_) => setState(() => loading = false));
          },
          iconBuilder: rollingIconBuilder,
          style: ToggleStyle(
            borderColor: Colors.transparent,
        ...
        AnimatedToggleSwitch<bool>.size(
          ...
          indicatorSize: const Size.fromWidth(100),
          customIconBuilder: (context, local, global) =>
              Text(local.value ? 'RAM' : 'CPU', style: TextStyle(color: Color.lerp(Colors.black, Colors.white, local.animationValue))),
          borderWidth: 4.0,
          iconAnimationType: AnimationType.onHover,
          style: ToggleStyle(
  ...
  Widget sizeIconBuilder(BuildContext context, AnimatedToggleProperties<int> local, GlobalToggleProperties<int> global) {
    return iconBuilder(local.value);
  }

  Widget alternativeIconBuilder(BuildContext context, AnimatedToggleProperties<int> local, GlobalToggleProperties<int> global) {
  ...
  Widget build(BuildContext context) {
    ...
    return CustomAnimatedToggleSwitch(
      ...
      wrapperBuilder: (context, global, child) => DecoratedBox(
          decoration: BoxDecoration(
              color: Color.lerp(Color.lerp(Colors.red, Colors.green, global.position), Colors.grey, global.loadingAnimationValue),
